# models/purchase_order_change_report_wizard.py
...
class PurchaseOrderChangeReportWizard(models.TransientModel):
    _name = 'purchase.order.change.report.wizard'
    _description = 'Purchase Order Change Report Wizard'
    ...
    def action_generate_excel_report(self):
        ...
        column_widths = [18, 18, 18, 18, 18, 50]
        ...
        worksheet.merge_range('B2:G4', "After Purchase Order Confirmation Changes", title_format)

        # logo_left_path = '/odoo18/custom/addons/microaccess_purchase/static/logo_left.png'
        # logo_right_path = '/odoo18/custom/addons/microaccess_purchase/static/logo_right.png'
        ...
        if logo_right_path and os.path.exists(logo_left_path):
            worksheet.insert_image('G2', logo_right_path, {
                'x_scale': 0.6, 'y_scale': 0.6, 'x_offset': 200, 'y_offset': 10, 'positioning': 1
            })

        headers = ['Order', 'Vendor', 'Order Date', 'Change Date', 'User', 'Details']
        header_row = 6

        for col, header in enumerate(headers, start=1):
            worksheet.write(header_row, col, header, header_format)
            worksheet.set_row(header_row, 38)
        ...
        filename = f'PurchaseOrderChangeReport_{self.date_from}_{self.date_to}.xlsx'
        ...
        return {
            ...
            'url': f'/web/content/?model=purchase.order.change.report.wizard&id={self.id}&field=file_data&filename_field=file_name&download=true',
            'target': 'new',
        }
